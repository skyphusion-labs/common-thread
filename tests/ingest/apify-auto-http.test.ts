...
describe('POST /ingest/apify', { timeout: 40_000 }, () => {
  it('ingests a YouTube Apify fixture and emits stylometric_youtube features', async () => {
    const created = await createInvestigation(testDb(), { id: uid('apify-auto-yt') });
    const fixture = (await import('../fixtures/youtube-posts.json')).default;
  ...
  it('returns 400 unsupported_export for a LinkedIn-shaped payload', async () => {
    const created = await createInvestigation(testDb(), { id: uid('apify-auto-li') });
    const res = await worker.fetch(
      new Request(`http://localhost/investigations/${created.id}/ingest/apify`, {
        method: 'POST',
        headers: {
  ...
  it('splits a mixed Twitter + YouTube upload on POST /ingest/apify', async () => {
    const created = await createInvestigation(testDb(), { id: uid('apify-mixed-yt') });
    const res = await worker.fetch(
      new Request(`http://localhost/investigations/${created.id}/ingest/apify`, {
        method: 'POST',
        headers: {
  ...
  it('ingests a Facebook public-page fixture and emits stylometric_facebook features', async () => {
    const created = await createInvestigation(testDb(), { id: uid('apify-auto-fb') });
    const fixture = (await import('../fixtures/facebook-posts.json')).default;
    const res = await worker.fetch(
      new Request(`http://localhost/investigations/${created.id}/ingest/apify`, {
        method: 'POST',
        headers: {
  ...
  it('splits a mixed Twitter + Facebook upload on POST /ingest/apify', async () => {
    const created = await createInvestigation(testDb(), { id: uid('apify-mixed-fb') });
    const res = await worker.fetch(
      new Request(`http://localhost/investigations/${created.id}/ingest/apify`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify([
          ...
          },
          {
            ...
            user: { id: '1000444', name: 'Finch Lowe' },
            url: 'https://www.facebook.com/finchlowe/posts/pfbid0abc',
          },
        ]),
      }),
      env
