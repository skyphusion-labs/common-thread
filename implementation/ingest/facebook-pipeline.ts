/**
 * Facebook public-page ingest + extraction pipeline.
 */
...
import { aggregateFacebookByAccount, extractFacebookHandles } from './facebook-parser';
import { FACEBOOK_ACCOUNT_EXTRACTORS, FACEBOOK_PAIR_EXTRACTORS } from './platform-extractors';
import type { TwitterIngestPipelineResult } from './pipeline';
import { parseTimestamp } from '../extractors/temporal/helpers';

export const APIFY_FACEBOOK_TIMELINE_TOOL = 'apify-facebook-timeline';
...
export interface RunFacebookIngestContext {
...
export async function runFacebookIngestPipeline(
  env: IngestPipelineEnv,
  ctx: RunFacebookIngestContext
): Promise<TwitterIngestPipelineResult> {
  const now = ctx.now ?? new Date().toISOString();
  const handles = extractFacebookHandles(ctx.payload);
  ...
  await manifest.append({
    ...
    collectionMethod: { tool: 'apify', version: '1', platform: 'facebook' },
  ...
  let listings = aggregateFacebookByAccount(ctx.payload);
  if (timeBounds) {
  ...
  const timelineArchive = await archiveAccountTimelines(archiveEnv, {
    ...
    collectedAt: now,
    timeBounds,
    tool: APIFY_FACEBOOK_TIMELINE_TOOL,
    platform: 'facebook',
    sourceFor: (account) => `https://www.facebook.com/${encodeURIComponent(account)}`,
  ...
  for (const handle of handles) {
    try {
      const basis = await packTextCell('Uploaded via Apify Facebook ingest', {
        key: ctx.encKey ?? null,
        investigationId: ctx.investigationId,
  ...
  const accountRuns = await runAccountExtractors(runnerEnv, {
    investigationId: ctx.investigationId,
    extractors: FACEBOOK_ACCOUNT_EXTRACTORS,
  ...
  if (handles.length >= 2) {
    pairRuns = await runPairExtractors(runnerEnv, {
      investigationId: ctx.investigationId,
      extractors: FACEBOOK_PAIR_EXTRACTORS,
