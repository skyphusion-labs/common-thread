/**
 * Bluesky ingest + extraction pipeline.
 */
...
import { aggregateBlueskyByAccount, extractBlueskyHandles } from './bluesky-parser';
import { BLUESKY_ACCOUNT_EXTRACTORS, BLUESKY_PAIR_EXTRACTORS } from './platform-extractors';
import type { TwitterIngestPipelineResult } from './pipeline';
import { parseTimestamp } from '../extractors/temporal/helpers';

export const APIFY_BLUESKY_TIMELINE_TOOL = 'apify-bluesky-timeline';
...
export interface RunBlueskyIngestContext {
...
export async function runBlueskyIngestPipeline(
  env: IngestPipelineEnv,
  ctx: RunBlueskyIngestContext
): Promise<TwitterIngestPipelineResult> {
  const now = ctx.now ?? new Date().toISOString();
  const handles = extractBlueskyHandles(ctx.payload);
  const archiveEnv = manifestBinding(env);
  const manifest = manifestStoreFor(archiveEnv, ctx.investigationId);

  await manifest.append({
    hash: ctx.rawHash,
    source: 'apify-bluesky',
    collectedAt: now,
    investigationId: ctx.investigationId,
    collectionMethod: { tool: 'apify', version: '1', platform: 'bluesky' },
  ...
  let listings = aggregateBlueskyByAccount(ctx.payload);
  if (timeBounds) {
  ...
  const timelineArchive = await archiveAccountTimelines(archiveEnv, {
    ...
    collectedAt: now,
    timeBounds,
    tool: APIFY_BLUESKY_TIMELINE_TOOL,
    platform: 'bluesky',
    sourceFor: (account) => `https://bsky.app/profile/${account}`,
  ...
  for (const handle of handles) {
    try {
      const basis = await packTextCell('Uploaded via Apify Bluesky ingest', {
        key: ctx.encKey ?? null,
        investigationId: ctx.investigationId,
  ...
  const accountRuns = await runAccountExtractors(runnerEnv, {
    investigationId: ctx.investigationId,
    extractors: BLUESKY_ACCOUNT_EXTRACTORS,
  ...
  if (handles.length >= 2) {
    pairRuns = await runPairExtractors(runnerEnv, {
      investigationId: ctx.investigationId,
      extractors: BLUESKY_PAIR_EXTRACTORS,
