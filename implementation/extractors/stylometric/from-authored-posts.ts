/**
 * Schema-parallel stylometric features from authored post texts (§4.3).
 * Shared by new-platform account extractors so pair extractors stay unchanged.
 */
...
export function stylometricFeaturesFromPosts(posts: AuthoredPost[]): ExtractedFeature[] {
  ...
  features.push({ category: cat, name: 'function_word_frequencies', value: { kind: 'json', value: fwFreq } });
  features.push({ category: cat, name: 'function_word_total', value: { kind: 'numeric', value: fwTotal } });
  features.push({
    category: cat,
  ...
  features.push({ category: cat, name: 'character_bigram_top50', value: { kind: 'json', value: top50Map } });
  features.push({
    category: cat,
  ...
  for (const tok of tokens) typeCounts.set(tok, (typeCounts.get(tok) ?? 0) + 1);
  let hapaxCount = 0;
  for (const c of typeCounts.values()) if (c === 1) hapaxCount++;
  features.push({ category: cat, name: 'token_count', value: { kind: 'numeric', value: totalTokens } });
  features.push({ category: cat, name: 'type_count', value: { kind: 'numeric', value: typeCounts.size } });
  features.push({
    category: cat,
    name: 'type_token_ratio',
    value: { kind: 'numeric', value: typeCounts.size / totalTokens },
  });
  features.push({ category: cat, name: 'hapax_legomena_count', value: { kind: 'numeric', value: hapaxCount } });
  features.push({
    category: cat,
  ...
  features.push({ category: cat, name: 'median_word_length', value: { kind: 'numeric', value: median(wordLengths) } });
  ...
  if (sentences.length > 0) {
    ...
    if (sentLengths.length > 0) {
      ...
      const variance = sentLengths.reduce((s, x) => s + (x - meanSent) ** 2, 0) / sentLengths.length;
      features.push({ category: cat, name: 'sentence_count', value: { kind: 'numeric', value: sentLengths.length } });
      features.push({ category: cat, name: 'mean_sentence_length', value: { kind: 'numeric', value: meanSent } });
      features.push({
        category: cat,
  ...
  features.push({ category: cat, name: 'uppercase_ratio', value: { kind: 'numeric', value: charRatios.uppercase } });
  features.push({ category: cat, name: 'digit_ratio', value: { kind: 'numeric', value: charRatios.digit } });
  features.push({ category: cat, name: 'punctuation_ratio', value: { kind: 'numeric', value: charRatios.punctuation } });
  const punctDist = countMajorPunctuation(allRawText);
  if (Object.keys(punctDist).length > 0) {
    features.push({ category: cat, name: 'punctuation_distribution', value: { kind: 'json', value: punctDist } });
  }
