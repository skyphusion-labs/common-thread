...
async function startIngest() {
  ...
  try {
    ...
    var data = await api('POST', '/investigations/' + encodeURIComponent(state.investigationId) + '/ingest/apify', { body: form });
  ...
  <div class="flex">
    ...
    <main class="flex-1 p-6 min-w-0">
      ...
      <section id="tab-upload" class="hidden">
        <h2 class="text-2xl font-semibold mb-1">Upload Apify data</h2>
        <p class="text-sm text-slate-600 mb-6">Upload Apify JSON exports for Twitter/X, Instagram, Reddit, Bluesky, Mastodon, TikTok, YouTube, or Facebook public pages (YouTube uses titles, descriptions, and comments). Mixed files are split by platform. The backend archives raw data and runs extractors.</p>
